...
function generateSummaryHtml(outbreaks) {
  ...
  const lines = [
    ...
    `            <h2>Potential Exposure (${sorted.monitoring.length} countries)</h2>`,
    `            <ul>`,
    ...
    `        <p class="summary-definitions"><strong>Definitions:</strong> <em>Confirmed</em> — laboratory-confirmed case. <em>Probable/Suspected</em> — meets clinical criteria, laboratory result pending or inconclusive. <em>Potential Exposure</em> — individuals with possible contact under health surveillance. <em>Affected country</em> — at least one case or exposed individual officially reported.</p>`,
    `      </div>`,
    `    </section>`,
  ];
